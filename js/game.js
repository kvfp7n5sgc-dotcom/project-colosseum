...
function renderDungeons(){if(!$("dungeonList"))return;$("dungeonList").innerHTML=GAME_DATA.dungeons.map(d=>{const lock=state.level<d.level&&state.profileType!=="gm";return`<article class="dungeon-card ${lock?"locked":""}"><h3>${d.name}</h3><p>${d.rooms.length} komnaty · poziom ${d.level}</p><p>Ukończenia: ${state.dungeonsCompleted[d.id]||0}</p><button class="primary-btn" data-dungeon="${d.id}" ${lock||state.dungeonRun?"disabled":""}>${lock?"Za niski poziom":`Rozpocznij — ${d.energy} wytrzymałości`}</button></article>`}).join("");document.querySelectorAll("[data-dungeon]").forEach(b=>b.onclick=()=>startDungeon(b.dataset.dungeon));renderDungeonRun();}
function startDungeon(id){const d=GAME_DATA.dungeons.find(x=>x.id===id);if(!d||state.dungeonRun)return;if(state.energy<d.energy&&state.profileType!=="gm"){$("dungeonReport").innerHTML="<p>Brakuje wytrzymałości.</p>";return;}if(state.profileType!=="gm")state.energy-=d.energy;state.dungeonRun={id,room:0,hp:combatPower().hp};persistAndRender();}
function renderDungeonRun(){const run=state.dungeonRun;if(!run){$("dungeonRunTitle").textContent="Brak aktywnego lochu";$("dungeonRunDescription").textContent="Wybierz loch.";$("dungeonProgress").innerHTML="";$("dungeonFightBtn").disabled=true;$("dungeonLeaveBtn").disabled=true;return;}const d=GAME_DATA.dungeons.find(x=>x.id===run.id),room=d.rooms[run.room];$("dungeonRunTitle").textContent=d.name;$("dungeonRunDescription").textContent=room?`Następna komnata: ${room.name}`:"Loch ukończony";$("dungeonProgress").innerHTML=d.rooms.map((x,i)=>`<div class="dungeon-step ${i<run.room?"done":i===run.room?"active":""}"><span>${i+1}</span><small>${x.name}</small></div>`).join("");$("dungeonFightBtn").disabled=!room;$("dungeonLeaveBtn").disabled=false;}
function fightDungeonRoom(){const run=state.dungeonRun;if(!run)return;const d=GAME_DATA.dungeons.find(x=>x.id===run.id),room=d.rooms[run.room],e=GAME_DATA.enemies.find(x=>x.id===room.enemy),p=combatPower();let ph=run.hp,eh=e.hp,r=0,log=[];while(ph>0&&eh>0&&r++<35){let c=Math.random()<.08+skillBonus("criticalEye")*.03,dm=Math.max(1,Math.round(p.attack*(.8+Math.random()*.4)));if(c)dm=Math.round(dm*1.75);eh-=dm;log.push(`Runda ${r}: zadajesz ${dm}${c?" obrażeń krytycznych":" obrażeń"}.`);if(eh<=0)break;let ed=Math.max(1,Math.round(e.attack*(.8+Math.random()*.4)-p.defense*.25));ph-=ed;log.push(`${e.name} zadaje ${ed} obrażeń.`);}if(ph>0){run.hp=ph;run.room++;log.unshift(`<strong>Komnata oczyszczona:</strong> ${room.name}.`);if(run.room>=d.rooms.length){let m=1+skillBonus("dungeonMastery")*.06,gold=Math.round(randBetween(d.rewardGold)*m*mountRewardMultiplier()),xp=Math.round(d.rewardXp*m*estateXpMultiplier());state.gold+=gold;state.xp+=xp;state.dungeonsCompleted[d.id]=(state.dungeonsCompleted[d.id]||0)+1;state.dungeonRun=null;log.unshift(`<strong>Loch ukończony!</strong> +${gold} złota, +${xp} XP.`);}}else{state.dungeonRun=null;if(skillBonus("secondWind"))state.energy=Math.min(state.maxEnergy,state.energy+1);log.unshift("<strong>Porażka. Wyprawa zakończona.</strong>");}$("dungeonReport").innerHTML=log.map(x=>`<p>${x}</p>`).join("");persistAndRender();}
function leaveDungeon(){state.dungeonRun=null;$("dungeonReport").innerHTML="<p>Opuszczono loch.</p>";persistAndRender();}
